...
let batteryCount = 3; // 電池格子數量
let timerInterval; // 計時器間隔
let elapsedTime = 0; // 經過的時間，以毫秒作單位
let scoreboard = [];
let rockSpeed = 250; // 落實初始速度，單位為毫秒
...
window.onload = function() {
    ...
    playButton.addEventListener('click', function() {
        ...
        generateRocks(); // 生成隨機數量的落石
        rockInterval = setInterval(moveRock, rockSpeed); // 設置落石初始速度
...
function increaseRockSpeed() {
    if (rockSpeed > 50) {
        ...
        clearInterval(rockInterval);
        rockInterval = setInterval(moveRock, rockSpeed);
    }
}
...
window.onload = function() {
    ...
    playButton.addEventListener('click', function() {
        ...
        playButton.style.display = 'none';
        generateRocks(); // 生成隨機數量的落石
        rockInterval = setInterval(moveRock, rockSpeed); // 設置落石的初始速度
        // 每隔3秒恢復一格電力
        batteryRecoverInterval = setInterval(() => {
            if (batteryCount < 3) {
                const batteryCells = document.querySelectorAll('#battery .cell');
